...
class Main {
    start() {
        ...
        periodicTableHtml.td = document.querySelectorAll('.periodic_table td');
        animator.addClickAction(periodicTableHtml.td);
    }
}
...
class PeriodicTable {
    ...
    pushElementTd(el) {
        let elementTd = '';
        elementTd += `<td data-x="${this.x}" data-y="${this.y}" data-type="${el.Type}">`;
        elementTd += `<span class='AtomicNumber'>${el.AtomicNumber}</span> `;
        elementTd += `<span class='Symbol'>${el.Symbol}</span><br>`;
        elementTd += `<span class='Element'>${el.Element}</span></td>`;
...
class UpperTable extends PeriodicTable {
    ...
    pushTableContainer(el) {
        while (this.x < el.Group) {
            super.pushEmptyTd();
        }
        super.pushElementTd(el);
    }
}
