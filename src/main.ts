...
class Main {
  start(): void {
    ...
    const tableContainer: { [index: string]: string } = generater.generateTableContainer(src);
...
class TableGenerater {
  ...
  public generateTableHeader(): string {
    const groupSize = 18;
    let tableHeader = '';
    for (let group = 1; group <= groupSize; ++group) {
      tableHeader += `<th>${group}</th>`;
    }
    return `<tr>${tableHeader}</tr>`;
  }

  public generateTableContainer(source: Atom[]): { [index: string]: string } {
    const upper = new UpperTable(1, 1);
    const lower = new LowerTable(1, 8);

    for (const atom of source) {
      if (atom.Type !== 'Lanthanide' && atom.Type !== 'Actinide') {
        upper.pushTableContainer(atom);
      } else {
        lower.pushTableContainer(atom);
      }
    }

    return {
      'upper': `<tr>${upper.tableContainer}</tr>`,
      'lower': `<tr>${lower.tableContainer}</tr>`,
    }
  }
}
...
class Animator {
  ...
  private color(y: number, x: number, colorName: string, isColor: boolean): void {
    if (x < 0 || x >= 18 || y < 0 || y >= 9) {
      return;
    }
...
class PeriodicTable {
  ...
  protected pushElementTd(atom: Atom): void {
    let elementTd = '';
    elementTd += `<td data-x="${this.x}" data-y="${this.y}" data-type="${atom.Type}">`;
    elementTd += `<span class='AtomicNumber'>${atom.AtomicNumber}</span> `;
    elementTd += `<span class='Symbol'>${atom.Symbol}</span><br>`;
    elementTd += `<span class='Element'>${atom.Element}</span></td>`;
    this.tableContainer += elementTd;
...
class UpperTable extends PeriodicTable {
  constructor(x: number, y: number) {
    super(x, y);
  }
  public pushTableContainer(atom: Atom): void {
    while (this.x < parseInt(atom.Group)) {
      super.pushEmptyTd();
    }
...
class LowerTable extends PeriodicTable {
  constructor(x: number, y: number) {
    super(x, y);
  }
  public pushTableContainer(atom: Atom): void {
